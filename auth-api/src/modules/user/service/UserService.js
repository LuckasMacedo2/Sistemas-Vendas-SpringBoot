...
class UserService { 
    ...
    validateAcessTokenData(email, password) {
        if(!email || !password) throw new UserException(httpStatus.UNAUTHORIZED, "Email and password must be informed");
    }

    async validatePassword(password, hashPassword) {
        if(!await bcrypt.compare(password, hashPassword)) throw new UserException(httpStatus.UNAUTHORIZED, "Password doesn't match.");
    }
}
